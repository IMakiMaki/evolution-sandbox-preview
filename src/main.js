...
let running = false
let lastSim = 0
let lastFullRenderTick = -1
let selectedSpeciesId = null
let previousSnapshot = new Map()
let world = createWorldFromControls()

function clamp(value, min, max) { return Math.max(min, Math.min(max, value)) }
function average(field) { return field.reduce((sum, value) => sum + value, 0) / Math.max(1, field.length) }
function pseudoNoise(seed, tick, index) { const x = Math.sin(seed * 12.9898 + tick * 78.233 + index * 37.719) * 43758.5453; return x - Math.floor(x) }
function lineageOf(species) { return species?.lineage || species?.id || 'unknown' }
function phenotypeTags(species) { const traits = species?.traits || {}; const tags = []; if ((traits.speed || 0) > 65) tags.push('长尾高速'); if ((traits.aggression || 0) > 62) tags.push('尖刺口器'); if ((traits.resistance || 0) > 62) tags.push('厚甲'); if ((traits.camouflage || 0) > 62) tags.push('拟态纹'); if ((traits.fertility || 0) > 72) tags.push('群聚繁殖'); if ((traits.metabolism || 0) > 68) tags.push('高代谢'); if (tags.length === 0) tags.push('基础表型'); return tags }
function snapshotWorld(world) { previousSnapshot = new Map(world.species.map(species => [species.id, { population: species.population, traits: { ...species.traits }, extinct: species.extinct, tier: species.tier }])) }
...
function createWorldFromControls() { const seed = Number(seedInput?.value || 42); const climateMode = climateSelect?.value || 'temperate'; const nextWorld = createInitialWorld({ seed, climateMode }); applyClimatePreset(nextWorld); return nextWorld }
function currentProvider() { return useMockLlm?.checked ? createMockStrategyProvider() : createRuleStrategyProvider() }
function currentInterval() { const slider = Number(speedRange?.value || 700); return clamp(1800 - slider, 120, 1800) }
...
function intensifyLateGameEvolution(world) {
  if (world.tick < 35 || world.tick % 3 !== 0) return []
  const messages = []
  const env = world.environment
  ...
  for (let index = 0; index < world.species.length; index += 1) {
    ...
    const chance = 0.018 + pressure * 0.012 + (species.tier || 0) * 0.003
...
function applyPopulationDynamics(world) {
  ...
  for (let index = 0; index < world.species.length; index += 1) {
    ...
    if (species.population <= 0) { species.extinct = true; species.agents = [] }
  }
  const latest = world.history[world.history.length - 1]
  if (latest) latest.species = world.species.map(species => ({ id: species.id, name: species.name, population: species.population, color: species.color }))
  if (shock) messages.push(shockSign > 0 ? '生态扰动：短期资源窗口让部分种群反弹。' : '生态扰动：环境压力造成多种群同步下滑。')
  return messages
}
function classifyEvent(text) { if (/分裂|分化/.test(text)) return ['物种分裂', '新的分支开始竞争']; if (/环境孕育|诞生/.test(text)) return ['环境孕育', '局部生态场出现新生命']; if (/突变|选择压力/.test(text)) return ['表型突变', '外形和行为倾向改变']; if (/围猎|击杀|死亡|灭绝/.test(text)) return ['生存事件', '种群结构改变']; if (/生态扰动|灾变|压力|冲击/.test(text)) return ['环境扰动', '生态场推动重洗牌']; return ['演化记录', '系统出现变化'] }
function isImportantEvent(text) { return /分裂|分化|环境孕育|诞生|选择压力|生态扰动|围猎|击杀|灭绝/.test(text) }
function pushEventCard(text, speciesId = null) {
  if (!eventCards || !text) return
  const [title, desc] = classifyEvent(text)
  const card = document.createElement('div')
  card.className = 'evo-card'
  card.innerHTML = `<strong>${title}</strong><p>${text}</p><div class="evo-meta"><span>Tick ${world.tick}</span><span>${desc}</span></div>`
  if (speciesId) card.onclick = () => { selectedSpeciesId = speciesId; renderSpeciesDetail() }
  eventCards.prepend(card)
  while (eventCards.children.length > 10) eventCards.removeChild(eventCards.lastChild)
}
function pushLog(container, text) { if (!container || !text) return; const item = document.createElement('div'); item.className = 'log-item'; item.textContent = text; container.prepend(item); while (container.children.length > 18) container.removeChild(container.lastChild) }
function clearLogs() { envLog.innerHTML = ''; speciesLog.innerHTML = ''; if (eventCards) eventCards.innerHTML = '' }
function renderSpeciesEditor() { if (!speciesEditor) return; const presets = getSpeciesPresets(); speciesEditor.innerHTML = presets.map(species => `<div class="species-pill"><span style="background:${species.color}"></span><div><strong>L${species.tier} ${species.name}</strong><small>${species.role}</small></div></div>`).join('') }
function mostActiveSpecies() { let best = world.species.find(species => !species.extinct && species.population > 0); let bestScore = -Infinity; for (const species of world.species) { if (!species || species.extinct || species.population <= 0) continue; const prev = previousSnapshot.get(species.id); const populationChange = Math.abs(species.population - (prev?.population ?? species.population)); const traitChange = Object.entries(species.traits || {}).reduce((sum, [key, value]) => sum + Math.abs(value - (prev?.traits?.[key] ?? value)), 0); const score = populationChange * 2 + traitChange + (species.splitProgress || 0) * 0.25 + (species.tier || 0); if (score > bestScore) { bestScore = score; best = species } } return best }
function renderSpeciesDetail() {
  ...
  if (!species) { speciesDetail.innerHTML = '<p class="muted">暂无存活物种。</p>'; return }
  selectedSpeciesId = species.id
  const tags = phenotypeTags(species)
  const rows = Object.entries(species.traits || {}).map(([key, value]) => `<div class="trait-row"><span>${key}</span><div class="trait-bar"><span style="width:${clamp(value,0,99)}%"></span></div><b>${Math.round(value)}</b></div>`).join('')
  speciesDetail.innerHTML = `<div class="detail-head"><div class="detail-avatar" style="background:${species.color}">L${species.tier}</div><div class="detail-title"><strong>${species.name}</strong><span>${species.role || ''}</span></div></div><div class="muted">谱系：${lineageOf(species)} ｜ 种群：${species.population} ｜ 分裂进度：${Math.round(species.splitProgress || 0)}</div><div class="tag-row">${tags.map(tag => `<span class="tag">${tag}</span>`).join('')}</div><div class="trait-grid">${rows}</div>`
}
function updateHud() { const env = world.environment; tickEl.textContent = world.tick; climateEl.textContent = `${env.climateLabel} / ${env.biomeLabel}`; resourceEl.textContent = `${Math.round(env.resources)}`; if (worldSummaryEl) { const living = world.species.filter(species => !species.extinct && species.population > 0).length; worldSummaryEl.textContent = `存活物种 ${living}｜植物 ${env.resourcePools.flora.toFixed(0)}｜孢子 ${env.resourcePools.spores.toFixed(0)}｜复杂度 ${env.complexity?.toFixed?.(0) ?? '-'}｜速度 ${currentInterval()}ms/步` } }
function renderAll(force=false) { updateHud(); if (force || world.tick % 3 === 0 || world.tick !== lastFullRenderTick) { if (force || world.tick % 4 === 0) renderSpeciesDetail(); drawWorld(canvas, world); if (force || world.tick % 3 === 0) drawTrend(trendCanvas, world.history); lastFullRenderTick = world.tick } }
function stepOnce() {
  snapshotWorld(world)
  ...
  if (world.tick % 2 === 0) pushLog(envLog, result.environmentNarrative)
  let pushed = 0
  for (const item of allNarratives) {
    if (pushed < 4 && world.tick % 2 === 0) { pushLog(speciesLog, item); pushed += 1 }
    if (isImportantEvent(item)) pushEventCard(item)
  }
  const latest = world.history[world.history.length - 1]
  if (latest) latest.species = world.species.map(species => ({ id: species.id, name: species.name, population: species.population, color: species.color }))
  renderAll(false)
}
function loop(timestamp) { if (!running) return; requestAnimationFrame(loop); if (timestamp - lastSim < currentInterval()) return; lastSim = timestamp; stepOnce() }
function resetWorld() { running = false; world = createWorldFromControls(); previousSnapshot = new Map(); selectedSpeciesId = null; lastSim = 0; lastFullRenderTick = -1; clearLogs(); pushLog(envLog, `初始化世界：种子 ${seedInput.value || 42}，气候 ${climateSelect.options[climateSelect.selectedIndex]?.text || climateSelect.value}。`); pushEventCard('世界初始化：环境场、生命等级和初始生态链已生成。'); renderAll(true) }

initBtn.onclick = resetWorld
stepBtn.onclick = () => { running = false; stepOnce() }
runBtn.onclick = () => { if (!running) { running = true; lastSim = 0; requestAnimationFrame(loop) } }
pauseBtn.onclick = () => { running = false }
climateSelect.onchange = resetWorld
seedInput.onchange = resetWorld
speedRange.oninput = () => renderAll(true)
useMockLlm.onchange = () => { pushLog(speciesLog, useMockLlm.checked ? '切换为 Mock LLM 策略层。' : '切换为规则策略层。') }
